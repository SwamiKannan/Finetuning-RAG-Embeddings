    ...
    for split in SPLITS:
        ...
        if split == 'train':
            final_finetune2(input_path=train_path, output_path=data_sciq.final_json,
                            ...
                            model_output=data_sciq.model_path, w_path=data_sciq.log_path,epochs=EPOCHS)
            if test_path:
                create_json_input(filename=test_path,
                                  output_file=data_sciq.final_json.replace('train','test'), # Remove train
